...
Tracking and shipment methods never retry automatically. If one returns `AMBIGUOUS_RESULT`, inspect the order's current remote state before considering a manual retry. Live mutation tests must use a deliberately selected current order under operator supervision and must never run in ordinary CI.
