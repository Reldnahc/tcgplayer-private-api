...
omits the trailing order-summary record, exposes the validated entries as
`PullSheetRow.orderAllocations`, and exposes their summed quantity as
`PullSheetRow.orderQuantity`. A legacy numeric-only allocation is associated
with the requested order when the export contains exactly one order; otherwise
its allocation list is empty rather than guessed.
